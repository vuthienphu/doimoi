...
class SolutionModuleTaskTemplateSkillRequirement(models.Model):
    _name = 'solution.module.task.template.skill.requirement'
    _description = 'Yêu cầu kỹ năng của Mẫu công việc'
    _order = 'sequence, id'
    ...
    # Tiện lợi khi cần lọc/nhóm theo Module giải pháp (liên quan gián tiếp qua mẫu công việc)
    module_id = fields.Many2one(
        related='task_template_id.module_id',
        string='Module giải pháp',
        store=True,
        readonly=True,
    )
    ...
    @api.constrains('task_template_id', 'skill_id')
    def _check_unique_skill_per_template(self):
        """Không cho khai báo trùng 1 kỹ năng nhiều lần trên cùng 1 mẫu công việc."""
        for record in self:
            duplicate = self.search([
                ('id', '!=', record.id),
                ('task_template_id', '=', record.task_template_id.id),
                ('skill_id', '=', record.skill_id.id),
            ], limit=1)
            if duplicate:
                raise ValidationError(_(
                    'Kỹ năng "%(skill)s" đã được khai báo cho mẫu công việc "%(template)s".',
                    skill=record.skill_id.name,
                    template=record.task_template_id.name,
                ))